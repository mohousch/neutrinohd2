...
static int writeData(void* _call)
{
	...
	while(1) 
	{
		PacketLength = (call->len - Position) <= MAX_PES_PACKET_SIZE ? (call->len - Position) : MAX_PES_PACKET_SIZE;

		int Remaining = call->len - Position - PacketLength;

		mpeg2_printf(20, "PacketLength=%d, Remaining=%d, Position=%d\n", PacketLength, Remaining, Position);

		HeaderLength = InsertPesHeader(PesHeader, PacketLength, 0xe0, call->Pts, 0);
		PacketStart = malloc(PacketLength + HeaderLength);
	...
	else if ((private_data || call->private_data) && must_send_header)
	{
		...
		while ((unsigned)pos <= data_len - 4)
		{
			...
			*/
			...
			//len = write(call->fd, ,call->len + codec_data_size, MPEG_VIDEO_PES_START_CODE);
		}
	}
