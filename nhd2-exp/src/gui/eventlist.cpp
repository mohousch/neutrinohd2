				...
				if(!evtlist[selected].description.empty())
				{
					...
					if(tmdb->getMovieInfo(evtlist[selected].description))
					{
						if ((!tmdb->getDescription().empty())) 
						{
							...
							CInfoBox * infoBox = new CInfoBox(g_Font[SNeutrinoSettings::FONT_TYPE_EPG_INFO1], ::SCROLL, &position, "", g_Font[SNeutrinoSettings::FONT_TYPE_MENU_TITLE], NEUTRINO_ICON_TMDB);
...
int CEventFinderMenu::showMenu(void)
{
	...
	*m_event = false;
	...
	ClistBoxWidget searchMenu(LOCALE_EVENTFINDER_HEAD, NEUTRINO_ICON_FEATURES);
