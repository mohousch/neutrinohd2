...
int CNeutrinoApp::loadSetup(const char * fname)
{
	...
	g_settings.mainmenu_design = configfile.getInt32("mainmenu_design", 0);
	// END OSD
...
void CNeutrinoApp::saveSetup(const char * fname)
{
	...
	configfile.setInt32("mainmenu_design", g_settings.mainmenu_design);
	// END OSD
	...
	while( true ) 
	{
		...
		if( (mode == mode_tv) || (mode == mode_radio) || (mode == mode_iptv) ) 
		{
			...
			else if( msg == CRCInput::RC_setup ) 
			{
				...
				if(g_settings.mainmenu_design == 0)
					_mainMenu.exec(NULL, "");
				else if(g_settings.mainmenu_design == 1)
					smartMenu();
